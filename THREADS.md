...
- [x] ~~Per-item main decider does not sync~~ / ~~Heir assignments do not sync~~ — **client shipped Sep 8 (`edf5146`); migration `0014` written, NOT yet applied.** Heirs are their own rows (`heir_assignments`: owner-only writes; members read only `revealed`; `after_death` hidden until the Phase-3 unlock; audited; realtime-filtered by RLS per device). `main_decider_name` on `items`, owner-gated by an additive trigger. Client is best-effort until `0014` is applied — item sync keeps working against the un-migrated project. **Needs the user: `supabase db push` (applies 0013 + 0014 + 0015 together).**
