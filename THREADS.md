...
Summary: 7 threads — 0 running, 3 idle (1, 6, 7), 4 retired and archived. Threads 6 and 7 hold **uncommitted** work awaiting the user's say-so (they were asked for features, not commits). **Migrations `0013`, `0014`, `0015` were applied to the linked project on Sep 8 (user-authorized, `supabase db push`)** — so the cloud schema is now ahead of what's committed: `rooms`, `roster_entries.is_admin`, `decline_invite()` exist server-side while their client code is still uncommitted. Harmless (nothing on `main` reads them) but the commit should follow soon. Thread 7's `notify-invite-declined` edge function is **not deployed** until someone runs `supabase functions deploy notify-invite-declined`.
