...
Deno.serve(async (req) => {
  ...
  // The secret lives in the database's Vault (generated by migration
  // 20260908000010, never written down anywhere else); check_digest_secret is
  // callable by the service role only. A DIGEST_SECRET function secret, if
  // set, is accepted as well.
  const given = req.headers.get('x-digest-secret');
  let authorized = false;
  if (given) {
    const override = Deno.env.get('DIGEST_SECRET');
    if (override && given === override) authorized = true;
    else {
      const { data } = await admin.rpc('check_digest_secret', { p_secret: given });
